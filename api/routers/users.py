...
async def create_user(user_request: UserCreateRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(db_user)
    return BaseResponse(message="User created successfully", data=db_user.user_id)
...
async def update_user(
    ...
    user.nickname = user_request.nickname
    db.commit()
...
async def update_user_goals(
    ...
    db.query(GoalCompany).filter(GoalCompany.user_id == user_id).delete()

    # Add new positions
    for position_id in request.position_ids:
        user_position = UserPosition(user_id=user_id, position_id=position_id)
        db.add(user_position)

    # Add new goal companies
    for company_id in request.company_ids:
        goal_company = GoalCompany(user_id=user_id, company_id=company_id)
        db.add(goal_company)
...
async def get_user_goal_companies(
    ...
    query = (db.query(Company)
             .join(GoalCompany)
             .filter(GoalCompany.user_id == user_id)
